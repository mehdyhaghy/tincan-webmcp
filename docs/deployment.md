...
- `setup.sh` installs nginx, certbot, ufw, and Bun, clones the repository into `/opt/tincan`, builds it as the unprivileged `tincan` user, issues a Let's Encrypt certificate, and starts the services.
